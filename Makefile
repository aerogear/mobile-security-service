...
.PHONY: build
build: setup
	go build -o $(BINARY) ./cmd/mobile-security-service-server/main.go

.PHONY: build_linux
build_linux: setup
	env GOOS=linux GOARCH=amd64 go build -o $(BINARY_LINUX_64) ./cmd/mobile-security-service-server/main.go
...
.PHONY: generate
generate:
	go generate ./cmd/mobile-security-service-server/main.go